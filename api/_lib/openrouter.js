 ...
 * Retries once on 429/5xx/timeouts only, never after a 2xx body was received.
 */
  ...
  while (attempt < 2) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        lastError = new ApiError(502, sanitizeProviderError(res.status, body), 'PROVIDER_ERROR', { status: res.status });
