...
import { costDeltaLabel, money, STRATEGY_HINT, STRATEGY_LABEL, zeroCostReason } from '../format';
import type { Evaluation, Finding, OptimizationExecution, OptimizationPlan } from '../types';
...
export function TestOpportunityDialog({ finding, onClose, onComplete }: { finding: Finding | null; onClose: () => void; onComplete: (evaluation: Evaluation) => Promise<void> | void }) {
  const { engine, selected, platformStatus, health, economics, setZevState } = useProduct();
  const { me } = useSession();
  ...
  const models = useMemo(() => platformStatus?.candidate_models || [], [platformStatus]);
  ...
  const blockers = useMemo(() => {
    ...
    if (strategy === 'model_substitution' && !providerReady) out.push('No model provider: the platform compute path is not configured.');
  ...
  const run = async () => {
    ...
    try {
      setPhase('plan');
      const plans = await engine.createPlans(selected.id, {
      ...
      setPhase('replay');
      const exec = await engine.executePlan(selected.id, created.id);
      setExecution(exec);
      ...
      const result = await engine.evaluate(selected.id, {
      ...
      {phase === 'configure' || phase === 'error' ? (
        <div className="grid gap-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {(['exact_reuse', 'model_substitution'] as const).map((s) => (
              <button key={s} type="button" onClick={() => setStrategy(s)} className={`plist-row is-clickable text-left ${strategy === s ? 'is-selected' : ''}`}>
          ...
          {strategy === 'model_substitution' && (
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Candidate model" hint={models.length ? 'From the allowlist for your compute path.' : 'No allowlist available; the engine refuses unknown models.'}>
                {models.length ? (
                  <select className="pselect" value={candidate} onChange={(e) => setCandidate(e.target.value)}>
                    {models.map((m) => <option key={m} value={m}>{m}</option>)}
                  </select>
                ) : (
                  <input className="pinput mono" value={candidate} onChange={(e) => setCandidate(e.target.value)} placeholder="openai/gpt-4o-mini" />
                )}
              </Field>
              <Field label="Spend cap for this replay (USD)" hint={health?.provider_mode === 'platform' ? `Charged to Zev credit · ${money(credit.remaining)} remaining` : 'Bounded by the pricing snapshot before any call is made.'}>
                <input className="pinput mono" type="number" min="0" step="0.05" value={budget} onChange={(e) => setBudget(e.target.value)} />
              </Field>
            </div>
          )}
          <div className="grid gap-3 sm:grid-cols-3">
            <Field label="Quality floor" hint="Candidate quality must reach this (0–1)."><input className="pinput mono" type="number" min="0.01" max="1" step="0.01" value={quality} onChange={(e) => setQuality(e.target.value)} /></Field>
            <Field label="Minimum samples"><input className="pinput mono" type="number" min="1" value={minSamples} onChange={(e) => setMinSamples(e.target.value)} /></Field>
            <Field label="Max latency regression %"><input className="pinput mono" type="number" min="0" value={latency} onChange={(e) => setLatency(e.target.value)} /></Field>
          </div>
          <label className="pcheckbox"><input type="checkbox" checked={fallback} onChange={(e) => setFallback(e.target.checked)} /><span><b>Require a safe fallback.</b><br />Verification fails unless the candidate can fall back to the current execution path.</span></label>
          <label className="pcheckbox"><input type="checkbox" checked={protect} onChange={(e) => setProtect(e.target.checked)} /><span><b>Require protected cases.</b><br />Runs without protected samples come back INCOMPLETE instead of VERIFIED.</span></label>
        ...
        <div className="grid gap-3">
          {steps.map(([key, title, desc], i) => {
            ...
            return (
              <div key={key} className={`pstep ${done ? 'is-done' : ''} ${active ? 'is-active' : ''}`}>
