...
import { Button, Card, CardHeader, Empty, Note, StatusChip, saveTextFile, useProductToast } from '../ui';
import { costDeltaLabel, dateTime, money, ms, num, observedLabel, pct, relativeTime, score, shortId, STRATEGY_LABEL, titleCase, zeroCostReason } from '../format';
...
export default function ExperimentsView() {
  const { selected, evaluations, executions, plans, experiments, findings, inspect, setInspect } = useProduct();
  ...
  const exportEvidence = async () => {
    ...
    const path = await saveTextFile(`zevqora-evidence-${shortId(current.id, 10)}.json`, JSON.stringify(payload, null, 2));
  ...
  return (
    <ProductFrame>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="peyebrow">Experiments</div>
          <h1 className="ph1">Baseline vs candidate. The gate decides.</h1>
          <p className="plede">Every candidate is replayed on your captured samples, graded deterministically and passed through cost, quality, latency and fallback gates. Cheaper alone never counts.</p>
        </div>
        <Button size="sm" onClick={() => dialogs.openTest(null)}><Play size={14} /> Test an opportunity</Button>
      </div>

      {!evaluations.length ? (
        <Empty title="No replays yet" description="Choose an opportunity and let Zev test it. You will see baseline vs candidate, the quality gate verdict and the full evidence trail here." action={<Button size="sm" onClick={() => navigate('/app/opportunities')}>Go to opportunities</Button>} />
      ) : (
        <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
          <div className="grid content-start gap-2">
            {evaluations.map((e) => {
              ...
              return (
                <button key={e.id} type="button" className={`plist-row is-clickable ${active ? 'is-selected' : ''}`} onClick={() => { setSelectedId(e.id); setInspect({ kind: 'evaluation', id: e.id }); }}>
      ...
      {experiments.length > 0 && (
        <Card className="mt-6">
          <CardHeader title="Experiment records" description="Every experiment row for this repository, including legacy imported candidate evidence." />
          <div className="pcard-body overflow-x-auto p-0">
            <table className="ptable">
              ...
              <tbody>
                {experiments.map((e) => (
                  <tr key={e.id} className="is-clickable" onClick={() => setInspect({ kind: 'experiment', id: e.id })}>
                    ...
                    <td className="whitespace-nowrap">{e.status === 'VERIFIED' && e.execution_proven && <Button size="xs" variant="secondary" onClick={(ev) => { ev.stopPropagation(); dialogs.openPrepare(e); }}><GitBranch size={12} /> Prepare change</Button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
