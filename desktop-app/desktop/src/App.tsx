...
export default function App() {
  ...
  const [welcomeComplete, setWelcomeComplete] = useState(localStorage.getItem('zevqora.welcomeComplete') === '1')
  ...
  const startBrowserLogin = async () => {
    ...
    try {
      ...
      await window.zevqoraDesktop.startBrowserAuth()
      setAuthLoading(false)
    } catch (error) {
  ...
  const enterWorkspace = () => {
    localStorage.setItem('zevqora.welcomeComplete', '1')
  ...
  const signOut = async () => {
    ...
    localStorage.removeItem('zevqora.welcomeComplete')
    setWelcomeComplete(false)
    setAuth(state)
  }

  if (!auth?.signedIn || !welcomeComplete) {
    return (
      <WelcomeScreen
        auth={auth}
        loading={authLoading}
        error={authError}
        onBrowserLogin={() => void startBrowserLogin()}
